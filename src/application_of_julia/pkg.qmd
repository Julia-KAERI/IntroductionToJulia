...
- `Pkg.jl` 을 이용한 환경과 패키지 개발의 기초에 대해서는 [환경과 패키지 개발](../ecosystem_of_julia/pkg_dev.qmd) 를 참고하세요. 이 글보다 먼저 읽어보는 것을 추천합니다.  
...
### 다른 사람의 프로젝트를 사용하기 {#sec-using_someone_elses_project}
...
### 임시 환경 {#sec-temporary_environments}
...
### 프로젝트 사전 컴파일 {#sec-environment_precompilation}
...
#### 자동 사전 컴파일 {#sec-automatic_precompilation}
...
### 패키지용 파일 생성 {#sec-generating_files_for_a_packages}
...
### 프로젝트에 의존성 추가 {#sec-adding_dependencies_to_the_project}
...
### 공개 API 를 정의하기 {#sec-defineing_a_public_api}
...
### 패키지에 빌드 단계를 추가하기 {#sec-adding_a_build_step_to_the_package}
...
### 패키지에 테스트 추가하기 {#sec-adding_test_to_the_package}
...
#### 테스트용 의존성 {#sec-test_specific_dependencies}
...
### 패키지 코드의 조건부 로딩 (Extensions) {#sec-conditional_loading_of_code_in_packages}
...
#### 확장의 동작 {#sec-behavior_of_extensions}
...
#### 하위 호환성 {#sec-backward_compatibility}
...
### 패키지 작명 가이드 {#sec-package_naming_rules}
...
### 패키지 등록 {#sec-registering_packages}
...
## 호환성 (Compatiblity) {#sec-compatibility}
...
### 버젼 지정 형식 {#sec-version_specifier_format}
...
#### `0` 으로 시작하는 버젼의 행동 (0.0.x 와 0.x.y) {#sec-behavior_of_versions_with_leading_zeros}
...
#### 부등호 지정자 {#sec-inequality_specifier}
...
### 레지스트리 관리 {#sec-managing_registries}
...
#### 레지스트리 제거 {#sec-removing_registry}
...
#### 레지스트리 업데이트 {#sec-updating_registry}
...
#### 레지스트리의 Compat.toml {#sec-registry_compat_toml}
...
#### 레지스트리 성향 (flavors) {#sec-registry_flavors}
...
#### 레지스트리 개설과 유지 {#sec-creating_and_maintaining_registries}
...
### 기본적인 사용법 {#sec-artifacts_basic_usage}
...
### `Artifacts.toml` 파일 {#sec-artifacts_toml_file}
...
### 아티팩트 타입과 프로퍼티 {#sec-artifact_types_and_properties}
...
### 아티팩트 위치 오버라이딩 {#sec-overriding_artifac_locations}
...
### 플랫폼 선택 확장 {#sec-expanding_platform_selection}
...
## `Project.toml` 과 `Manifest.toml` {#sec-project_toml_manifest_toml}
...
### `Manifest.toml` {#sec-maifest_toml}
...
#### 서로 다른 Julia 버젼에서의 다른 Manifetsts 들 {#sec-different_manifests_for_different_julia_versions}
...
#### `Manifest.toml` 항목들 {#sec-manifest_toml_entries}
...
#### 브랜치로 추가된 패키지 {#sec_added_package_by_branch}
...
#### 커밋(commit) 을 통해 추가된 패키지 {#sec-added_package_by_commit}
...
#### 같은 이름을 가진 복수의 패키지 {#sec-multiple_packages_with_the_same_name}
