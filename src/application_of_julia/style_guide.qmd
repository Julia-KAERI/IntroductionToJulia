...
## 과도하게 타입을 특정하는것을 피한다 {#sec-avoid_writing_overly-specific_types}
...
## 과도한 인자의 다양성은 함수 내부가 아니라 함수를 호출할 때 처리한다 {#sec-handle_excess_argument_diversity_in_the_caller}
...
## 인자를 변경하는 함수의 이름 끝에 `!` 를 붙인다 {#sec-append_exclamation_at_the_end_of_function_name}
...
## 이상한 타입 `Unions` 은 피한다 {#sec-avoid_strange_type_unios}

`Union{Function, AbstractString}` 과 같은 타입은 대개 설계가 더 분명해야 한다는 것을 의미합니다.

</br>

## 복잡한 컨테이너 타입을 피한다 {#sec-avoid_elaborate_container_types}
...
## 직접적으로 필드에 접근하기보다는 export 된 메서드를 사용한다 {#sec-prefer_exported_methods_over_direct_field_access}
...
## Julia `base/` 와 일치하도록 이름을 붙인다 {#sec-using_naming_convections_with_julia_base}
...
## 함수 인자의 순서를 Julia Base 와 비슷하게 한다 {#sec-write_functions_with_argument_ordering_simmilar_to_julia_base}
...
## `try-catch` 를 남용하지 않는다 {#sec-dont_overuse_try_catch}

`try-catch` 로 에러를 탐지하는 것보다 피하는 것이 낫습니다.

</br>

## 조건문을 괄호로 싸지 않는다 {#sec-dont_parrenthesize_conditions}

Julia 는 `if` 나 `while` 문에서 괄호를 요구하지 않습니다. `if (a == b)` 대신에 `if a == b` 를 사용하세요.

</br>

## `...` 를 남용하지 않는다 {#sec-don_overuse_splicing}
...
## 생성자가 바로 그 타입을 반환하는지 확인한다 {#sec-ensure_constructors_return_type}
...
## 불필요한 정적 매개변수를 사용하지않는다 {#sec-dont_use_unnecessary_static_parameters}
...
## 인스턴스와 타입에 대한 혼동을 피한다 {#sec-avoid_confusion_whether_between_something_is_an_istance_or_type}
...
## 매크로를 남용하지 않는다 {#sec-dont_overuse_macro}
...
## 인터페이스 레벨에서 안전하지 않은 동작을 노출하지 않는다 {#sec-don_expose_unsafe_operations_at_the_interface_level}
...
## 기본 콘테이너 타입의 메서드를 재정의(overload) 하지 않는다 {#sec-dont_overload_methods_of_base_containers}
...
##  타입이 동등성에 조심할 것 {#sec-be_careful_with_type_equality}
...
## 이름이 붙여힌 함수 `f` 에 대해 `x->f(x)` 와 같은 익명함수를 작성하지 말것 {#sec-dont_write_a_trivial_anonymous_function_for_a_named_function}
...
## 가능하다면 generic 코드의 숫자형 리터럴에 부동소수를 피하라 {#sec-avoid_using_float_for_numeric_literals_in_generic_code}

수를 다루는 generic code 를 작성한다면, 그리고 많은 수 타입의 인자와 함께 실행되어야 한다면 타입 프로모션에 의해 가장 덜 영향을 받는 수 타입의 리터럴을 사용하는 것이 좋다.
...
이다. 차이가 보이는가? 첫번째(`f(x)`) 에서는 `2.0` 이라는 부동소수형 리터럴을 사용한 반면에 두번째(`g(x)`) 에서는 `2` 라는 `Int` 형 리터럴을 사용했다. 따라서 `f` 는 대부분의 인자 타입에 대해 결과가 부동소수이지만 `g` 의 경우는 인자 타입을 보존한다. 이것은 서로 다른 두 수 타입을 곱할 때 타입 프로모션이 발생하며 타입 프로모션 규칙에 의해 `Float64` 가 `Int64` 보다 타입 교란적이다. (`A` 타입과 `B` 타입의 연산 결과가 `A` 타입이 되면 `A` 를 타입 교란적이라고 한다.) `Rational` 은 `Int64` 보다 타입 교란적이지만 `Float64` 보다는 덜 타입 교란적이므로 정수가 아닌 수를 사용한다면 `Rational` 을 사용해보라.
